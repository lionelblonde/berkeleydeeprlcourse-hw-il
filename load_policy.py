...
def load_policy(filename):
    """The policies were pickled individually.
    For each, a dictionary was created, containing 2 keys: nonlineraity type and policy type.
    For the latter, the key is for example 'GaussianPolicy' and the value is the policy parameters.
    For the nonlinearity, the key is always equal to the string 'nonlin_type'.
    The policy_params (value of the key 'policy_type') is itself a dictionary, with keys:
    'logstdevs_1_Da', 'hidden', 'obsnorm' and 'out'.
    When unpickled, we get the dictionary.

    Example of dictionary:

    data = {
        'nonlin_type': 'leaky_relu',
        'GaussianPolicy': {
            'logstdevs_1_Da': 'blabla'
            'hidden': {
                'FeedforwardNet': {
                    'Layer1': {
                        'AffineLayer': {
    ...
    def build_policy(obs_bo):
        ...
        def apply_nonlin(x):
            if nonlin_type == 'leaky_relu':
                return U.leaky_relu(x, leak=.01)  # openai/imitation nn.py:233
    ...
    policy_fn = U.function([obs_bo], a_ba)
